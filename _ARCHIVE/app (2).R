  ...
  tabPanel("Univariate Results",
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        HTML("<p><center><h4><b>For review purposes only: DO NOT CITE. Please send all questions, concerns, and suggestions to <a href='mailto:jessecaputo@eco.umass.edu'>jessecaputo@eco.umass.edu</a>.</b></h4></center></p>")
  ...
  navbarMenu("More",
             tabPanel("About",
                      ...
                      sidebarLayout(
                        ...
                        sidebarPanel(
                          selectInput("yall", "Download complete summary dataset, by year", unique(et$YEAR)),
                      ...
                      mainPanel(
                        ...
                        HTML(paste("<p><b>Suggested citation: Caputo, J. and B. Butler. National Woodland Owner Survey (NWOS) Dashboard (BETA). Accessed ", Sys.Date(), ".</b></p>",sep=""))
                      )
                    )
                  ),
             tabPanel("Help",
...
server <- function(input, output, session) {
  ...
  #subset et by year only (for export)
  ...
  cref <- function(){
    ...
    if (!grepl("NOT PUBLISHED",ets$TNUM[1])){
      paste("This data corresponds to Table",ets$TNUM[1],"in: Butler Brett J.; Butler, Sarah M.; Caputo, Jesse; Dias, Jaqueline; Robillard, Amanda; Sass, Emma M. In review. Family Forest Ownerships of the United States, 2018: Results from the USDA Forest Service, National Woodland Owner Survey. Res. Bull. NRS-XX. Madison, WI: U.S. Department of Agriculture, Forest Service, Northern Research Station. [ADD DOI]")
    }
  ...
  output$downloadJSON <- downloadHandler(filename = "NWOSdashboard_EXPORT.json",
                                            content = function(file) { 
                                              write_json(subets(),file,pretty=T)
  ...
  output$downloadJSONMap <- downloadHandler(filename = "NWOSdashboard_EXPORT.json",
                                         content = function(file) { 
                                           write_json(subets.map(),file,pretty=T)
