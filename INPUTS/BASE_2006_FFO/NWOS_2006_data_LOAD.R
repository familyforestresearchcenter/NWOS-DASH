...
rg <- read.csv("INPUTS/BASE_2006_FFO/REF_GEO.csv")
rg$GEO_ABB[rg$GEO_LEVEL %in% c('REGION','SUBREGION')] <- paste('US',rg$GEO_ABB[rg$GEO_LEVEL %in% c('REGION','SUBREGION')],sep="_")
# rt <- read.csv("INPUTS/BASE_2006_FFO/REF_TABLE.csv")
# rv <- read.csv("INPUTS/BASE_2006_FFO/REF_VARIABLE.csv")
rl <- read.csv("INPUTS/BASE_2006_FFO/REF_LABEL.csv")
...
# #write.csv(rtz,"INPUTS/BASE_2006_FFO/REF_TABLE_VIZ.csv",row.names=F,na='')

rtz <- read.csv("INPUTS/BASE_2006_FFO/REF_TABLE_VIZ.csv") #!!!!!!!!!!!!!!!!!!just edit this one
  
#loading estimates
et <- readRDS("INPUTS/BASE_2006_FFO/NWOS_2006_FFO_ONE_PLUS.RDS")
et$DOMAIN <- "1+"
et$STRATUM <- "FFO"
et$YEAR <- "2006"
et$TNUM <- 'NOT PUBLISHED'

et2 <- readRDS("INPUTS/BASE_2006_FFO/NWOS_2006_FFO_TEN_PLUS.RDS")
et2$DOMAIN <- "10+"
et2$STRATUM <- "FFO"
et2$YEAR <- "2006"
et2$TNUM <- 'NOT PUBLISHED'

et3 <- readRDS("INPUTS/BASE_2006_FFO/NWOS_2006_FFO_HUNDRED_PLUS.RDS")
et3$DOMAIN <- "100+"
et3$STRATUM <- "FFO"
et3$YEAR <- "2006"
et3$TNUM <-'NOT PUBLISHED'

et4 <- readRDS("INPUTS/BASE_2006_FFO/NWOS_2006_FFO_THOUSAND_PLUS.RDS")
et4$DOMAIN <- "1000+"
et4$STRATUM <- "FFO"
et4$YEAR <- "2006"
...
save(list=c("et"),file="INPUTS/BASE_2006_FFO/BASE_2006_FFO_DATA.RData")
