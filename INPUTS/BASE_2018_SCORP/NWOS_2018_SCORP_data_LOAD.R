...
rg <- read.csv("INPUTS/BASE_2018_SCORP/REF_GEO.csv")
rg$GEO_ABB[rg$GEO_LEVEL %in% c('REGION','SUBREGION')] <- paste('US',rg$GEO_ABB[rg$GEO_LEVEL %in% c('REGION','SUBREGION')],sep="_")
# rt <- read.csv("INPUTS/BASE_2018_SCORP/REF_TABLE.csv")
# rv <- read.csv("INPUTS/BASE_2018_SCORP/REF_VARIABLE.csv")
rl <- read.csv("INPUTS/BASE_2018_SCORP/REF_LABEL.csv")
# 
...
# #write.csv(rtz,"INPUTS/BASE_2018_SCORP/REF_TABLE_VIZ.csv",row.names=F,na='')

rtz <- read.csv("INPUTS/BASE_2018_SCORP/REF_TABLE_VIZ.csv") #!!!!!!!!!!!!!!!!!!just edit this one
  
#loading estimates
et <- readRDS("INPUTS/BASE_2018_SCORP/NWOS_2018_SCORP_TEN_PLUS.RDS")
et$DOMAIN <- "10+"
et$STRATUM <- "SCORP"
et$YEAR <- "2018"
et$TNUM <- 'NOT PUBLISHED'

et2 <- readRDS("INPUTS/BASE_2018_SCORP/NWOS_2018_SCORP_HUNDRED_PLUS.RDS")
et2$DOMAIN <- "100+"
et2$STRATUM <- "SCORP"
et2$YEAR <- "2018"
et2$TNUM <-'NOT PUBLISHED'

et3 <- readRDS("INPUTS/BASE_2018_SCORP/NWOS_2018_SCORP_ONE_PLUS.RDS")
et3$DOMAIN <- "1+"
et3$STRATUM <- "SCORP"
et3$YEAR <- "2018"
et3$TNUM <- 'NOT PUBLISHED'

et4 <- readRDS("INPUTS/BASE_2018_SCORP/NWOS_2018_SCORP_THOUSAND_PLUS.RDS")
et4$DOMAIN <- "1000+"
et4$STRATUM <- "SCORP"
et4$YEAR <- "2018"
...
save(list=c("et"),file="INPUTS/BASE_2018_SCORP/BASE_2018_SCORP_DATA.RData")
