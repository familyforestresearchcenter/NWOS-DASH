...
rg <- read.csv("INPUTS/BASE_2021_FFO/REF_GEO.csv")
rg$GEO_ABB[rg$GEO_LEVEL %in% c('REGION','SUBREGION')] <- paste('US',rg$GEO_ABB[rg$GEO_LEVEL %in% c('REGION','SUBREGION')],sep="_")
# rt <- read.csv("INPUTS/BASE_2021_FFO/REF_TABLE.csv")
# rv <- read.csv("INPUTS/BASE_2021_FFO/REF_VARIABLE.csv")
rl <- read.csv("INPUTS/BASE_2021_FFO/REF_LABEL.csv")
# 
...
# #write.csv(rtz,"INPUTS/BASE_2021_FFO/REF_TABLE_VIZ.csv",row.names=F,na='')

rtz <- read.csv("INPUTS/BASE_2021_FFO/REF_TABLE_VIZ.csv") #!!!!!!!!!!!!!!!!!!just edit this one
  
#loading estimates
et <- readRDS("INPUTS/BASE_2021_FFO/NWOS_2021_FFO_TEN_PLUS.RDS")
et$DOMAIN <- "10+"
et$STRATUM <- "FFO"
et$YEAR <- "2021"
et$TNUM <- 'NOT PUBLISHED'

et2 <- readRDS("INPUTS/BASE_2021_FFO/NWOS_2021_FFO_HUNDRED_PLUS.RDS")
et2$DOMAIN <- "100+"
et2$STRATUM <- "FFO"
et2$YEAR <- "2021"
et2$TNUM <-'NOT PUBLISHED'

et3 <- readRDS("INPUTS/BASE_2021_FFO/NWOS_2021_FFO_ONE_PLUS.RDS")
et3$DOMAIN <- "1+"
et3$STRATUM <- "FFO"
et3$YEAR <- "2021"
et3$TNUM <- 'NOT PUBLISHED'

et4 <- readRDS("INPUTS/BASE_2021_FFO/NWOS_2021_FFO_THOUSAND_PLUS.RDS")
et4$DOMAIN <- "1000+"
et4$STRATUM <- "FFO"
et4$YEAR <- "2021"
...
save(list=c("et"),file="INPUTS/BASE_2021_FFO/BASE_2021_FFO_DATA.RData")

#combine with existing estimates table
et.2021 <- et #rename
load("DEPLOY/NWOS_dashboard_DATA.RData") #load old
et <- rbind(et.2021,et) #combine

et <- et[et$INC==1,] #drop those not to include

#removing unnecessary word "Primary" from metadata across surveys
ind <- which(grepl("primary forest management decisions maker",et$DESCRIPTION))
et$DESCRIPTION[ind] <- "forest management decision maker"
et$TABLE[ind] <- "Decision maker"
